...
1) Предисловие
2) Введение в opaque predicate
3) классификация opaque predicate
4) рассмотрение групп contextual opaque predicate по семантики исполнения CFG.
...
Прежде чем начать что такое opaque predicate(непрозрачные предикаты),рассмотрим что такое обфускация.
Обфускация - это раздел криптографии который изучает методы и принципы затреднения или не возможности статического анализа и динамического анализа семантики программы.
Обфускация имеет 5 ветвей или подразделов изучающих обфусцирование семантики
1) obfuscation binary semantic
2) obfuscation CFG semantic
3) obfuscation data semantic
4) obfuscation format file semantic

Я рассмотрю в этой статье 2 подраздел обфускации (науки),а именно из за головка не прозрачные предикаты.

***Введение в opaque predicate***

Прежде чем рассмотрить ,что такое не opaque predicate ,нужно понять ,что такое predicate(предикат).
Предикат или прозрачный предикат - это выражение имеющее булевую семантику и возращающие либо всегда какое то булевое значение ,либо возращая true или false ,в зависимости от входных аргументов и параметров.Семантика предиката прозрачного понятна на этапе компиляции и не требует тчательного анализа в отличие от opaque predicate.
Рассмотрим opaque predicate.
opaque predicate (непрозрачный предикат) - это предикат семантику которого является не тривиальной и сложной или даже не возможной при статическом анализе и затрудняющем сильно динамический анализ.
opaque preducate - это самый сложный вид CFG обфускации при разработке и при анализе и оценки семантики обфусцированной этой технологией программы.
...
Invariant opaque predicate - (Invariant переводится как без вариантов ,этот термин взят из математики,он означает что: переменная имеющая какое то значение до операций ,будет иметь такое значение после математических операций ) это непрозрачный предикат,который либо всегда при любом условии или аргументов в логическом выражение возращает true или всегда false.
пример:
x*x+1<0 это выражение всегда будет возращать ложь ,потому что при любом x, правая часть неравенства будет больше нуля и положительно.
Рассмотрим второй тип.
Contextual opaque predicate (Contextual или контекстный здесь показывает отражает ,что непрозрачный предикат зависит от контекста) - это непрозрачный предикат который зависит от своего контекста ,а именно от аргументов,параметров,событий которые могут влиять на логическо выражение или на сам предикат ,этот тип не прозрачного предиката,в отличие от инвариантого непрозрачного предиката,может true или false возращать ,в зависимости от контекста ,который оказывает влияние на предикат.
Пример:
int func(int param1)
{
	if(-param1%2)
